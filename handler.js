       ...
       /* if (opts['queque'] && m.text && !(isMods || isPrems)) {
		...
		//m.reply('*ERROR DE COMANDO*')}
               ...
               let hl = _prefix 
                ...
                let jeff = `${plugins.botAdmin || plugins.admin || plugins.group || plugins || noPrefix || hl ||  m.text.slice(0, 1) == hl || plugins.command}`
                if (adminMode && !isOwner && !isROwner && m.isGroup && !isAdmin && jeff) return   
               ...
               if (plugin.rowner && plugin.owner && !(isROwner || isOwner)) { // Both Owner
                ...
                if (!isPrems && plugin.money && global.db.data.users[m.sender].money < plugin.money * 1) {
                    this.reply(m.chat, `🦓 𝙉𝙊 𝙏𝙄𝙀𝙉𝙀 𝙅𝙀𝙁𝙁𝘾𝙊𝙄𝙉𝙎`, m)
                    continue     
		}
		    ...
		    continue // If the level has not been reached
                ...
                try {
                 ...
                 if (m.money)
                        m.reply(+m.money + ' 𝙅𝙀𝙁𝙁𝘾𝙊𝙄𝙉𝙎 🦓 𝙐𝙎𝘼𝘿𝙊(𝙎)')
...
let prep = generateWAMessageFromContent(m.chat, { extendedTextMessage: { text: msg, contextInfo: { externalAdReply: { title: lenguajeGB.smsAvisoAG().slice(0,-2), body: [wm, '🐺 𝗦𝘂𝗽𝗲𝗿 ' + gt + ' 🐺', '🐼 sademisad1.gmail.com'].getRandom(), thumbnail: jeffImg.getRandom(), sourceUrl: [md, yt, ig, paypal, fb].getRandom() }}}}, tg)
